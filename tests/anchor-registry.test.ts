...
describe("anchor-registry", () => {
  ...
  it("counts entries", async () => {
    const chain = new Chain(); const [d] = chain.accounts.values() as Account[];
    chain.mineBlock([Tx.contractCall("anchor-registry","add-entry",[types.buff(val)],d.address),Tx.contractCall("anchor-registry","add-entry",[types.buff(Buffer.from("dd".repeat(32),"hex"))],d.address)]);
