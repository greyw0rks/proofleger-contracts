;; anchor-registry.clar
;; Errors: u100 not-authorized, u101 not-found, u102 already-exists, u103 invalid-input
(define-constant CONTRACT-OWNER tx-sender)
(define-map anchor_registry_entries { entry-id: uint } { owner: principal, value: (buff 32), block: uint, active: bool })
...
(define-public (add-entry (value (buff 32)))
  (let ((id (+ (var-get entry-nonce) u1)))
    ...
    (ok (map-set anchor_registry_entries { entry-id: id } { owner: tx-sender, value: value, block: block-height, active: true }))))
...
(define-public (update-entry (id uint) (value (buff 32)))
  (let ((e (unwrap! (map-get? anchor_registry_entries { entry-id: id }) (err u101))))
    (asserts! (is-eq tx-sender (get owner e)) (err u100))
    (ok (map-set anchor_registry_entries { entry-id: id } (merge e { value: value })))))
